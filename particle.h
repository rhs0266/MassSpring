...
const double coeff = 0.1; // coefficient for collision
...
struct Particle{
	...
	void move(){ // change position to next frame
		...
		for (int i=0;i<connect.size();i++){
			...
			V2 vec = diff / len;
			V2 f = -(k_s[i] * (len - rest) + k_d * (v - q->v).dot(vec)) * vec;
...
void push_edge(Particle *x, Particle *y, double rest, double k_s = 0.1){
...
void Initialize(){
	...
	for (int t=0;t<N*N;t++){
		int i = t/N, j = t%N;
		particles[t]=Particle(V2(-50 * (N/2),100) + V2(50,0)*i + V2(0,50)*j, V2(0,0), 0.5 / (N*N), t, 0);
		for (int k=0;k<4;k++){
		...
		for (int k=0;k<4;k++){
			...
			if (i2<0 || j2<0 || i2>=N || j2>=N) continue;
			int t2 = i2 * N + j2;
			push_edge(&particles[t], &particles[t2], 50 * 2, 0.7);
		}
	}
...
void drawParticles(){
	for (int i=0;i<N*N;i++){
		if (particles[i].fixed==1) glColor3f(255,0,0);
